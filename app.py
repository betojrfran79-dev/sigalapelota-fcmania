...
st.markdown("""
    ...
    /* Botão de Aplicar Filtros (Vermelho) */
    ...
    .st-emotion-cache-p5m40 {
        border-bottom: 1px solid #30363d;
    }
    
    /* Zoom nas imagens da tabela */
    [data-testid="stDataFrame"] img {
        transform: scale(1.15);
    }
    
    /* Estilo do Box de Agradecimento */
    .agradecimento-box {
        background-color: #15181b;
...
def obter_miniface(player_id):
    possiveis_caminhos = [
        f"heads/p{player_id}.png", f"heads/{player_id}.png",
        f"heads/p{player_id}.dds", f"heads/{player_id}.dds"
    ]
...
if st.session_state.jogador_selecionado is None:
    ...
    with st.sidebar.form("form_filtros"):
        busca_nome = st.text_input("Nome", "")
        todas_nacionalidades = sorted(df['nationality'].dropna().unique().tolist())
        filtro_nacionalidade = st.multiselect("Nacionalidade (ID)", todas_nacionalidades)
        todos_times = sorted(df['teamname'].dropna().unique().tolist())
        filtro_clube = st.multiselect("Clube", todos_times)
        todas_posicoes = sorted(df['Position'].dropna().unique().tolist())
        filtro_posicao = st.multiselect("Posição", todas_posicoes)

        with st.expander("Físico & Perfil Básico", expanded=True):
        ...
        with st.expander("Categoria Ofensivo"):
        ...
        with st.expander("Categoria Habilidade"):
        ...
        with st.expander("Categoria Movimentação"):
        ...
        with st.expander("Categoria Força"):
        ...
        with st.expander("Categoria Mentalidade"):
        ...
        with st.expander("Categoria Defesa"):
            hab_defensiva = st.slider("Habilidade Defensiva", 1, 99, (1, 99))
            dividida_pe = st.slider("Dividida em Pé", 1, 99, (1, 99))
            carrinho = st.slider("Carrinho", 1, 99, (1, 99))

        with st.expander("Categoria Goleiro"):
